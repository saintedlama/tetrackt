...
func TestOscillatorSilent(t *testing.T) {
	...
	osc := NewOscillator(Silent, 440.0, sr, 0, 0, 0)
...
func TestOscillatorSquare(t *testing.T) {
	...
	osc := NewOscillator(Square, 100.0, sr, 0, 0, 0)
...
func TestOscillatorSawtooth(t *testing.T) {
	sr := beep.SampleRate(44100)
	osc := NewOscillator(Sawtooth, 100.0, sr, 0, 0, 0)
...
func TestOscillatorSawtoothReverse(t *testing.T) {
	sr := beep.SampleRate(44100)
	osc := NewOscillator(SawtoothReverse, 100.0, sr, 0, 0, 0)
	samples := streamN(osc, 10)
...
func TestOscillatorTriangle(t *testing.T) {
	...
	// phase=0 → 4*0-1 = -1
	s0 := streamN(NewOscillator(Triangle, 100.0, sr, 0.0, 0, 0), 1)
	if math.Abs(s0[0][0]-(-1.0)) > 1e-6 {
		t.Errorf("phase 0.0: want -1, got %v", s0[0][0])
	}

	// phase=0.25 → 4*0.25-1 = 0
	s25 := streamN(NewOscillator(Triangle, 100.0, sr, 0.25, 0, 0), 1)
	if math.Abs(s25[0][0]) > 1e-6 {
		t.Errorf("phase 0.25: want 0, got %v", s25[0][0])
	}

	// phase=0.5 → -4*0.5+3 = 1
	s50 := streamN(NewOscillator(Triangle, 100.0, sr, 0.5, 0, 0), 1)
...
func TestOscillatorSine(t *testing.T) {
	...
	for _, tt := range tests {
		s := streamN(NewOscillator(Sine, 100.0, sr, tt.phase, 0, 0), 1)
...
func TestOscillatorNoise(t *testing.T) {
	sr := beep.SampleRate(44100)
	osc := NewOscillator(Noise, 440.0, sr, 0, 0, 0)
...
func TestOscillatorStereo(t *testing.T) {
	sr := beep.SampleRate(44100)
	osc := NewOscillator(Sine, 440.0, sr, 0, 0, 0)
	samples := streamN(osc, 100)
	for i, s := range samples {
		if s[0] != s[1] {
...
func TestOscillatorInitialPhase(t *testing.T) {
	...
	osc := NewOscillator(Square, 100.0, sr, 0.6, 0, 0)
	s := streamN(osc, 1)
	if s[0][0] != -1.0 {
