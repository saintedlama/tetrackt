...
Rows that have custom Ticks, ARP, Continuous, or an active Effect set look identical to plain rows in the current renderer. Apply a subtle accent to the row number (similar to the existing playback highlight) on any row that carries non-default effects, making patterns easy to scan at a glance.
...
| Field      | Type              | Default                | Non-default condition           |
| ---------- | ----------------- | ---------------------- | ------------------------------- |
| `Ticks`    | `int`             | `0` (use global ticks) | `Ticks != 0`                    |
| `Continuous` | `bool`          | `false`                | `Continuous == true`            |
| `Arpeggio` | `ArpeggioEffect`  | empty                  | `Arpeggio.IsActive()` is true   |
| `Effect`   | `TrackerEffect`   | `{Type: EffectNone}`   | `Effect.Type != EffectNone`     |

`Note` and `Volume` are intentionally excluded — the feature targets playback-behaviour modifiers, not pitch/amplitude data.

`Effect` is a newer field (type alias for `effects.Effect` from `ui/tracker/effects_compat.go`). It holds an inline effect type and parameter set via the FX and PARAM subcolumns. `EffectNone` is the zero value.
...
4. `row.Effect.Type != EffectNone` — an inline effect command is set
...
The style is **not** bold, keeping it visually quieter than the cursor and playback highlights.

### Step 2 — Add the helper predicate

Add a package-private function in `ui/tracker/tracker.go`:

```go
// rowHasEffects reports whether any track at row index r carries non-default
// effects (custom Ticks, Continuous mode, an active Arpeggio, or an inline Effect).
func rowHasEffects(r int, tracks []Track) bool {
    for _, t := range tracks {
        row := t.Rows[r]
        if row.Ticks != 0 || row.Continuous || row.Arpeggio.IsActive() || row.Effect.Type != EffectNone {
            return true
        }
    }
    return false
...
No new types or imports are needed — `Track`, `TrackRow`, `audio.ArpeggioEffect.IsActive()`, and `EffectNone` are already in scope.
...
Row 07 (has FX)      → yellow (#ffb300), normal weight   ← new
...
**`Ticks == 0` semantics**: `NewTracker` initialises rows with zero-value `TrackRow{}`, so `Ticks == 0` correctly represents "use global ticks" (the `defaultRowTicks` constant, currently `6`). Only a non-zero per-row override is flagged.

**`Effect.Type == EffectNone` semantics**: `EffectNone` is the zero value of `effects.Type`, so a freshly initialised row is not flagged. Any inline effect command set via the FX subcolumn will be non-zero.
...
**Future effects fields**: When new `TrackRow` effect fields are added, the `rowHasEffects` predicate should be extended at the same time.
# Plan: Per-Row Visual Markers for Non-Default State
